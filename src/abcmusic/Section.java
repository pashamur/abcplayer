...
public class Section implements ABCmusic{
    ...
    private static class SectionException extends RuntimeException {
        public SectionException(String message) {
            super("SectionException: "+message);
        }
    }
    /**
     ...
     * @throw SectionException if start with bar or have empty measure
     */
    public Section(List<Token> tokens) {
        ...
        if (numTokens<1) 
        	throw new SectionException("Empty section.");
        
        if (tokens.get(numTokens-1).type.equals(Token.Type.simple_bar))
            throw new SectionException("Extra simple-bar.");
